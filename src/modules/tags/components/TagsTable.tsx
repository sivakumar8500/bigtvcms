...
import { AutoAwesome, Delete, Edit, CheckCircle, RadioButtonUnchecked } from '@mui/icons-material';
import { Tag } from '../domain/tags.model';

const tagColors = [
  '#ff4081', '#00e5ff', '#ffeb3b', '#76ff03', '#e040fb',
  '#ff9100', '#651fff', '#3d5afe', '#00e676', '#ff1744',
];
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{
        ...
        <Box sx={colStyle(3.0)}>{t.colName}</Box>
      ...
      {/* Data Rows */}
      {paginatedData.length > 0 ? paginatedData.map((tag, idx) => {
        const color = tagColors[idx % tagColors.length];
        const tagLanguages = [
          { code: 'EN', present: !!tag.tagEn?.trim() },
          { code: 'TE', present: !!tag.tagTe?.trim() },
          { code: 'HI', present: !!tag.tagHi?.trim() },
          { code: 'ML', present: !!tag.tagMl?.trim() },
        ];
            ...
            <Box sx={{
              ...
              <Box sx={{ ...colStyle(3.0), gap: 2 }}>
                <Box sx={{
                  ...
                  {tag.imageUrl ? (
                    <Box component="img" src={tag.imageUrl} sx={{ width: '100%', height: '100%', borderRadius: '10px', objectFit: 'cover' }} />
                  ) : (
                    <AutoAwesome sx={{ fontSize: '1rem', color }} />
                  )}
                </Box>
                <Typography variant="body2" sx={{ color: isDark ? '#ffffff' : '#1c1445', fontWeight: 600, fontSize: '0.95rem' }}>
                  {(language === 'te' && tag.tagTe) ||
                   (language === 'hi' && tag.tagHi) ||
                   (language === 'ml' && tag.tagMl) ||
                   tag.tagEn || tag.aitagname}
                </Typography>
              </Box>
              ...
              <Box sx={colStyle(1.5)}>
                <Chip
                  icon={(tag.isActive ?? true) ? <CheckCircle sx={{ fontSize: '0.9rem !important' }} /> : <RadioButtonUnchecked sx={{ fontSize: '0.9rem !important' }} />}
                  label={(tag.isActive ?? true) ? t.active : t.inactive}
                  size="small"
                  sx={{
                    fontWeight: 600, fontSize: '0.72rem',
                    backgroundColor: (tag.isActive ?? true)
                      ? (isDark ? 'rgba(102,187,106,0.15)' : 'rgba(102,187,106,0.1)')
                      : (isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)'),
                    color: (tag.isActive ?? true) ? '#66bb6a' : (isDark ? '#d0caeb' : '#9e9e9e'),
                    borderColor: (tag.isActive ?? true) ? '#66bb6a44' : 'transparent',
                    border: '1px solid',
              ...
              <Box sx={colStyle(1.2)}>
                <Switch
                  ...
                  onChange={() => toggleActive(tag.aitagid)}
                  size="small"
                  sx={{
              ...
              <Box sx={{ ...colStyle(1.5), gap: 1 }}>
                ...
                <IconButton
                  size="small"
                  onClick={() => handleDeleteClick(tag.aitagid)}
                  sx={{
                    color: '#f44336',
        ...
        <Box sx={{ p: 6, textAlign: 'center' }}>
          <AutoAwesome sx={{ fontSize: '3rem', color: isDark ? '#d0caeb' : '#bbb', mb: 1 }} />
